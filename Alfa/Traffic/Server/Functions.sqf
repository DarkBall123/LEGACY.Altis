/*
 * Alfa/Traffic/Server/Functions.sqf
 * Defines road selection, edge-road discovery, and connectivity helpers for traffic spawning.
 */
...
ENGIMA_TRAFFIC_DeleteAllWaypointsFromGroup = {
	...
	private _waterPositionsInARow = 0;
		...
		while { _stepDistance * _i < _totDistance && { _waterPositionsInARow < 3 } } do {
...
ENGIMA_TRAFFIC_FindSpawnSegment = {
	...
	ENGIMA_TRAFFIC_RoadsConnected = {
		...
		if (_thisSegment == _targetSegment) then {
			...
			{
				if (!(_x in _visitedSegments) && { [_x, _targetSegment, _visitedSegments] call ENGIMA_TRAFFIC_RoadsConnected }) exitWith {
					_connected = true;
				};
